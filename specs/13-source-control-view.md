...
## Behavior
- Sources: workspace scope → `[working, staged, ...recentCommits]`; commit/range scope → one synthetic source `{kind:"commit"|...}` — simplest: for commit scope use the commit item built from `review.metadata`, for range a `working`-shaped item is wrong; instead the view accepts `initialSourceId` and, for non-workspace scope, renders the source list with a single row for `review.scope` (id `commit:<oid>` or `range`) and hides `RECENT COMMITS`.
